...
public interface RestaurantSummaryEntityRepository extends JpaRepository<RestaurantEntity, Long> {
    ...
    @Query(
            "SELECT new woowa.team4.bff.domain.RestaurantSummary(r.uuid, r.name, '', COALESCE(rs.reviewCount, 0), COALESCE(rs.averageRating, 0.0), CAST(COALESCE(GROUP_CONCAT(m.name), '') AS string)) "
